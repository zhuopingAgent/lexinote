...
import {
  ...
  type ActivePracticeGenerationType,
...
10. 只允许生成选择题或中译日；不得生成改错、语体改写、自由场景回应等其他形式。
...
type PromptDefinition = {
  id: string;
  version: number;
  exerciseType: ActivePracticeGenerationType;
  rules: string[];
...
const PROMPTS: Record<ActivePracticeGenerationType, PromptDefinition> = {
...
export function buildPracticeGenerationPromptV2(input: {
  ...
  if (!isActivePracticeGenerationType(input.intent.exerciseType)) {
    throw new Error(`Unsupported generated exercise type: ${input.intent.exerciseType}`);
  }
...
export function getPracticePromptDefinition(exerciseType: ActivePracticeGenerationType) {
