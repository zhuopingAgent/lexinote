...
7. grammar 使用带「〜」的词典式规范形式，一个候选只表示一个可独立教学的语法结构，不能把词汇或相邻结构拼进语法名称。例如「かもしれませんので」应按价值选择「〜かもしれない」和/或「〜ので」，不能输出「〜かもしれませんので」；「見たほうがよさそうです」中的建议语法应写成「〜たほうがいい」；「〜に変更してもよろしいでしょうか」应写成「〜てもよろしいでしょうか」。
