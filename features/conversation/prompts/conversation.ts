...
const MODE_GUIDANCE: Record<ConversationMode, string> = {
  auto: "根据输入自动判断任务。没有明确指令的完整中文句子按中译日处理，完整日语句子按日译中处理；只有用户明确要求时才润色、纠错或讲解。主要内容不是日语学习时，简短说明本助手专注于中日语言学习。在餐厅、医院等涉及过敏、症状或安全确认的场景，保留全部关键限制，省略生硬的主语，并用对方可直接回答的礼貌确认请求。中译日时必须把中文词完整转换为日语，不能把「花生」等中文词原样混入日语；花生使用「ピーナッツ」或「落花生」。",
  zh_to_ja:
    "把用户的中文完整翻译成自然、可直接使用的日语，不能残留只在中文中使用的词。自然译文放在最前面。准确判断请求中的动作主体：请求对方执行动作时使用「〜ていただけますか」等请求表达；只有询问自己或己方是否可以执行时才使用「〜てもよろしいでしょうか」。涉及花生过敏时使用「ピーナッツアレルギー」或「落花生アレルギー」，禁止写成「花生アレルギー」。",
  ja_to_zh: "把用户的日语翻译成自然、准确的中文，并保留语气和隐含含义。",
  polish_ja:
    "将用户日语改成自然表达。先给完整修改稿，再用中文简要说明关键改动。敬语类别必须说明准确，例如「お水」的「お」是美化语，不能解释为对水表示尊敬。",
  explain_ja:
...
6. vocabulary 必须使用能独立查词的词典原形，禁止把包含已提取语法的整句活用形（例如「試してみます」）再标为 vocabulary；expression 仅用于意义不可直接组合推导的固定搭配或惯用语，不要把「先に」等普通单词或副词当固定表达，也不要收集助手给出的普通改写、礼貌变体或语法例句。不要把孤立的「には」「必要です」「〜があります」「〜が入っていないか」或「ひとつの/一つの」等普通存在形式、上下文从句、数量修饰语当作 grammar；除非用户明确询问这些形式，否则优先选择同句中更有学习价值的具体词汇。
7. grammar 使用带「〜」的词典式规范形式，不能把过去式、礼貌式直接当作语法名，例如「〜てもらいました」必须写成「〜てもらう」、「〜ことになっています」必须写成「〜ことになっている」。一个候选只表示一个可独立教学的语法结构，不能使用「/」「／」拼接多个形式，也不能把词汇、标点变化或相邻结构拼进语法名称；「来週、この問題について」这类上下文片段不是语法；「おっしゃる」「いらっしゃる」等特殊敬语动词属于 vocabulary，不是 grammar；「アレルギーがある」等搭配属于 expression，但不能把带「。」「！」「？」的整句保存为 expression；名词を直接接「いただけますか」时也不是「〜ていただけますか」语法。例如「かもしれませんので」应按价值选择「〜かもしれない」和/或「〜ので」，不能输出「〜かもしれませんので」；「見たほうがよさそうです」中的建议语法应写成「〜たほうがいい」；「〜に変更してもよろしいでしょうか」应写成「〜てもよろしいでしょうか」；「〜に変更していただけますか」应写成「〜ていただけますか」。
