...
const MODE_GUIDANCE: Record<ConversationMode, string> = {
  chat:
    "作为通用助手理解并完成用户当前请求。可以讨论编程、写作、生活规划、知识解释和语言学习等主题；使用用户当前使用的语言回答。不要把没有明确翻译意图的中文或日语自动改写成另一种语言。遇到不确定或时效性信息时明确说明边界，不编造事实。",
...
export function buildConversationSystemPrompt(input: {
  ...
  const assistantRole =
    input.mode === "chat"
      ? "你是 LexiNote 的通用对话助手。先准确完成用户的请求；日语学习能力只是可按需调用的专长，不是每轮对话的强制方向。"
      : "你是 LexiNote 的中日语言学习助手，服务对象是中文母语的日语学习者。";
...
2. 通用对话模式使用用户当前使用的语言；中日专项模式默认使用中文解释，日语译文保持自然，并根据场景选择合适语体。
3. 不输出词汇候选、记忆建议或数据库操作，这些由后续分析完成。
4. 不声称已经保存任何词汇、语法或记忆。
5. 仅在用户要求翻译时应用翻译约束：默认只给主要译文，不主动改写原句、不把其他说法称为“更自然”，也不在结尾追问；不得添加原文没有的请求、建议、结论或下一步，自然化只能调整语序、语体和主语省略；把原文已经表达的确认、请求或许可意图转换成目标语言中可直接使用的问句不属于添加内容，必须保留其沟通功能；只有存在会误导用户的重要歧义时才补一句说明。
...
export const CONVERSATION_ANALYSIS_SCHEMA: Record<string, unknown> = {
  ...
  required: ["overview", "learning_items"],
  additionalProperties: false,
};

export const CONVERSATION_MAINTENANCE_SCHEMA: Record<string, unknown> = {
  type: "object",
  properties: {
  ...
  required: ["title", "summary", "memories"],
  additionalProperties: false,
};

export function buildConversationAnalysisPrompt(input: {
  messages: ConversationMessage[];
  focus?: "all" | "grammar" | "vocabulary" | "expressions";
  instruction?: string;
}) {
  ...
  const focusGuidance = {
    all: "综合检查高价值语法、词汇和固定表达。",
    grammar: "只提取语法候选；不要返回词汇或固定表达。",
    vocabulary: "只提取可独立查词的词汇；不要返回语法或固定表达。",
    expressions: "只提取固定表达、搭配或惯用语；不要返回普通词汇或语法。",
  }[input.focus ?? "all"];

  return `按用户意图分析下面一轮对话中的日语学习价值，返回严格符合 schema 的 JSON。

关注范围：${focusGuidance}
用户补充意图：${input.instruction?.trim() || "无；按默认标准分析。"}

当前一轮：
${transcript}

规则：
1. overview 用简洁中文直接回应用户的分析意图，说明最值得注意的学习结论；没有合适内容时明确说明，不要为了填充而制造候选。不要复述系统规则或分析过程。
2. 学习项只从“当前一轮”提取，不读取或复述其他历史轮次；只保留对中文母语日语学习者真正有价值的词汇、固定表达或语法，总数最多 5 个，通常只选 1–3 个，不要求填满，宁缺毋滥。同一个语言现象只选一个 kind：能由语法规则完整解释时优先 grammar，不要再把其普通活用句作为 expression。句中出现可独立查词的专业复合名词时，应优先保留为 vocabulary，例如「在職証明書」「住民票」「領収書」；用户明确询问某个词的含义时必须提取该 vocabulary。
3. 用户输入很短时也要检查其中完整的补助动词和复合语法结构，不能因为表达自然或已经给出回答就跳过，也不要误拆成单独的「ます形」；但候选形式必须能从本轮 source_excerpt 的实际文字中得到直接支持。
4. 用户明确询问某个语法形式时，必须把该形式作为 grammar 学习项；日语纠错时应提取可绑定语法库的通用规则（例如「い形容词过去形」「な形容词过去形」），不要把「楽しいです/楽しかったです」或「静かかったです/静かでした」这种错误与正确形式的对照串当作语法名称。
5. vocabulary 必须使用能独立查词的词典原形，禁止把包含已提取语法的整句活用形再标为 vocabulary；expression 仅用于意义不可直接组合推导的固定搭配、惯用语或强依赖场景的完整回应（例如服务场景中表示婉拒的「大丈夫です」「結構です」），不要把这些完整回应标成普通 vocabulary。不要把「先に」等普通单词或副词当固定表达，也不要收集助手给出的普通改写、礼貌变体或语法例句。不要把孤立的「には」「必要です」「〜があります」「〜が入っていないか」或「ひとつの/一つの」等普通存在形式、上下文从句、数量修饰语当作 grammar；除非用户明确询问这些形式，否则优先选择同句中更有学习价值的具体词汇。
6. grammar 使用带「〜」的词典式规范形式，不能把过去式、礼貌式直接当作语法名，例如「〜てもらいました」必须写成「〜てもらう」、「〜ことになっています」必须写成「〜ことになっている」。一个候选只表示一个可独立教学的语法结构，不能使用「/」「／」拼接多个形式，不能使用「→」「⇒」输出错误与正确形式的对照串，也不能把词汇、标点变化或相邻结构拼进语法名称；「来週、この問題について」这类上下文片段不是语法；「おっしゃる」「いらっしゃる」等特殊敬语动词属于 vocabulary，不是 grammar；「お水」中的「お」是美化语前缀，整个「お水」最多归为 vocabulary，不能标成 grammar，也不能解释为对水表示尊敬；「アレルギーがある」等搭配属于 expression，但不能把带「。」「！」「？」的整句保存为 expression；名词を直接接「いただけますか」时也不是「〜ていただけますか」语法。例如「かもしれませんので」应按价值选择「〜かもしれない」和/或「〜ので」，不能输出「〜かもしれませんので」；「見たほうがよさそうです」中的建议语法应写成「〜たほうがいい」；「〜に変更してもよろしいでしょうか」应写成「〜てもよろしいでしょうか」；「〜に変更していただけますか」应写成「〜ていただけますか」。
7. source_excerpt 必须逐字来自本轮对话，不得把规范语法形式当作原文引用。
8. meaning_zh 和 explanation_zh 必须使用自然中文，不得混入韩文，不能用 medical、business 等英文类别词代替中文场景名称。grammar 的 reading 必须为 null；接续说明是语法解释的一部分，不能单独生成学习项。
9. 严格遵守“关注范围”和用户补充意图。用户要求排除某项、比较某个表达、只看自己的原句或加深某类说明时，overview 和候选都必须体现该意图。`;
}

export function buildConversationMaintenancePrompt(input: {
  sessionTitle: string;
  ...
  const transcript = input.messages
    .map((message) => `${message.role === "user" ? "用户" : "助手"}：${message.content}`)
    .join("\n\n");
  return `维护一段通用对话的标题、摘要和记忆建议，返回严格符合 schema 的 JSON。
...
规则：
1. summary 用中文压缩稳定上下文、用户意图和已得到的结论，不超过 1000 字；不要记录无关寒暄、系统规则、分析过程或学习项分类。
2. 只有标题不是手动设置且此前摘要为空时才给不超过 20 字的简洁标题，否则 title 为 null。
3. memories 不是摘要。只有用户明确透露、稳定且可能影响未来回答的偏好、目标或场景背景才建议；普通提问通常返回空数组。跨会话稳定偏好用 global，仅本会话背景用 session。
4. 禁止把“用户说了什么、助手回答了什么、使用了哪些规则”等对话过程保存为记忆，也不要把临时事实或助手自己的结论保存为记忆。`;
}
