...
5. vocabulary 必须使用能独立查词的词典原形，禁止把包含已提取语法的整句活用形（例如「試してみます」）再标为 vocabulary；expression 仅用于意义不可直接组合推导的固定搭配或惯用语，不要把「先に」等普通单词或副词当固定表达，也不要收集助手给出的普通改写、礼貌变体或语法例句。
6. grammar 使用带「〜」的词典式规范形式，一个候选只表示一个可独立教学的语法结构，不能把相邻结构拼成不存在的语法点。例如「かもしれませんので」应按价值选择「〜かもしれない」和/或「〜ので」，不能输出「〜かもしれませんので」。
7. source_excerpt 必须逐字来自本轮对话，不得把规范语法形式当作原文引用。
8. memories 不是对话摘要。只有用户明确透露、稳定且可能影响未来回答的偏好、目标或场景背景才建议为 memory；普通翻译或讲解通常返回空数组。跨会话稳定偏好用 global，仅本会话背景用 session。
9. 禁止把“用户说了什么、助手回答了什么、这轮使用了哪些规则、提取了哪些学习项”等对话过程写成 memory；不要把临时翻译内容、词汇知识或助手自己的结论保存为任何记忆。`;
}
