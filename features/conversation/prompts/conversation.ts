...
7. memories 不是对话摘要。只有用户明确透露、稳定且可能影响未来回答的偏好、目标或场景背景才建议为 memory；普通翻译或讲解通常返回空数组。跨会话稳定偏好用 global，仅本会话背景用 session。
8. 禁止把“用户说了什么、助手回答了什么、这轮使用了哪些规则、提取了哪些学习项”等对话过程写成 memory；不要把临时翻译内容、词汇知识或助手自己的结论保存为任何记忆。`;
}
