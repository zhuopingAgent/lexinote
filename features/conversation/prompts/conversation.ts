...
const MODE_GUIDANCE: Record<ConversationMode, string> = {
  auto: "根据输入自动判断任务。没有明确指令的完整中文句子按中译日处理，完整日语句子按日译中处理；只有用户明确要求时才润色、纠错或讲解。主要内容不是日语学习时，简短说明本助手专注于中日语言学习。在餐厅、医院等涉及过敏、症状或安全确认的场景，保留全部关键限制，省略生硬的主语，并用对方可直接回答的礼貌确认请求。",
  zh_to_ja:
...
7. grammar 使用带「〜」的词典式规范形式，不能把过去式、礼貌式直接当作语法名，例如「〜てもらいました」必须写成「〜てもらう」、「〜ことになっています」必须写成「〜ことになっている」。一个候选只表示一个可独立教学的语法结构，不能把词汇、标点变化或相邻结构拼进语法名称；「来週、この問題について」这类上下文片段不是语法；「おっしゃる」「いらっしゃる」等特殊敬语动词属于 vocabulary，不是 grammar；「アレルギーがある」等搭配属于 expression；名词を直接接「いただけますか」时也不是「〜ていただけますか」语法。例如「かもしれませんので」应按价值选择「〜かもしれない」和/或「〜ので」，不能输出「〜かもしれませんので」；「見たほうがよさそうです」中的建议语法应写成「〜たほうがいい」；「〜に変更してもよろしいでしょうか」应写成「〜てもよろしいでしょうか」；「〜に変更していただけますか」应写成「〜ていただけますか」。
