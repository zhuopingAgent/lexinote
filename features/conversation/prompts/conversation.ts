...
6. vocabulary 必须使用能独立查词的词典原形，禁止把包含已提取语法的整句活用形（例如「試してみます」）再标为 vocabulary；expression 仅用于意义不可直接组合推导的固定搭配或惯用语，不要把「先に」等普通单词或副词当固定表达，也不要收集助手给出的普通改写、礼貌变体或语法例句。不要把孤立的「には」「必要です」当作 grammar；除非用户明确询问这些形式，否则优先选择同句中更有学习价值的具体词汇。
