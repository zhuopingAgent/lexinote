...
规则：
1. summary 用中文压缩稳定上下文和本轮结论，不超过 1000 字，不记录无关寒暄；只写对后续对话有用的内容，禁止复述本提示中的规则、候选数量限制、提取策略或分析过程。
...
5. 用户明确询问某个语法形式时，必须把该形式作为 grammar 学习项；日语纠错时应提取可绑定语法库的通用规则（例如「い形容词过去形」），不要把「楽しいです/楽しかったです」这种错误与正确形式的对照串当作语法名称。
...
7. grammar 使用带「〜」的词典式规范形式，一个候选只表示一个可独立教学的语法结构，不能把相邻结构拼成不存在的语法点。例如「かもしれませんので」应按价值选择「〜かもしれない」和/或「〜ので」，不能输出「〜かもしれませんので」；「見たほうがよさそうです」中的建议语法应写成「〜たほうがいい」，不能输出「ほうがよさそうです」。
