  ...
  return (
    <header className="app-header border-b border-black/50 bg-[#1e1e1ecc]">
      <div className="app-header__inner mr-auto flex h-[clamp(56px,6vw,60px)] w-full max-w-[1160px] items-center gap-[clamp(24px,5vw,48px)] pl-[clamp(16px,4vw,32px)] pr-[clamp(16px,3vw,24px)]">
        <Link
          href={brandHref}
          className="app-brand flex min-h-11 min-w-11 items-center justify-center gap-2.5"
        >
        ...
        >
          {navItems.map((item) => {
            const className = item.active
              ? "inline-flex h-11 min-w-11 items-center justify-center text-[clamp(13px,1.8vw,16px)] text-white/60"
              : "inline-flex h-11 min-w-11 items-center justify-center text-[clamp(13px,1.8vw,16px)] text-white/45 transition hover:text-white/68";
