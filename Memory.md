...
- Grammar practice is session-based: `practice_sessions` own ordered `exercise_instances`; each `practice_attempt` produces `mastery_evidence` and updates one `learner_skill_states` row for the concrete grammar sense and skill dimension. The six dimensions are meaning discrimination, form/connection, contrast selection, register control, contextual production, and transfer/naturalness.
- Active exercise APIs must not expose `expected_features`, reference answers, the full hint ladder, or grammar examples. Reference answers become public only after a correct attempt or explicit reveal. Failed text feedback is persisted in legacy full form but returned without correction text until reveal.
- Pedagogical planning is deterministic and lives under `features/grammar-learning/domain/`; AI may realize planned text exercises but must not choose the target skill, exercise type, difficulty, or answer policy. Legacy `POST /api/practice/generate` and `/submit` remain available for compatibility.
