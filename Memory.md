...
- User-triggered analysis receives only the target assistant answer and its parent user message, plus a focus and optional free-form instruction. Each run creates a `conversation_analyses` version. A successful revision becomes current and dismisses superseded unconfirmed candidates, while saved learning items remain. Grammar candidates still require direct current-turn evidence and server-side sense resolution; high-confidence grammar reconciliation suppresses duplicate inflected expressions.
