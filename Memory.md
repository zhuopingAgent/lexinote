...
- [Conversation Testing](docs/ai/CONVERSATION_TESTING.md)
...
- Conversation is available under `/conversation` as a general ChatGPT-style text assistant, with explicit Chinese/Japanese translation, polishing, and explanation quick modes. Keep its service/repository/prompt logic under `features/conversation/`; route handlers remain thin.
- Conversation application workflows are separated into `ConversationSessionService`, `ConversationMessageService`, `ConversationMaintenanceService`, `ConversationAnalysisService`, and `ConversationLearningService`. Depend on the capability interfaces in `application/ports.ts`; do not rebuild a single catch-all conversation service.
- Conversation has three independent flows: `defaultTeacher` streams the answer; a background `cheap` maintenance call updates only title, summary, and memory suggestions; a `cheap` Japanese-learning analysis runs only after a per-message action or `/analysis` command. A failure in either secondary flow must not invalidate the answer.
- User-triggered analysis receives only the target assistant answer and its parent user message, plus a focus and optional free-form instruction. Each run creates a `conversation_analyses` version protected by a renewable lease token. Candidate inserts, failure, and completion must be fenced by that token; completion locks the shared message inside a transaction before selecting the current revision. A successful newer revision outranks any older worker that finishes later, dismisses superseded unconfirmed candidates, and preserves saved learning items.
- Conversation model context includes structured preferences, active global memories, active memories for the current session, the current session summary, and at most 16 completed messages/16,000 characters through the target user message. Resolve that boundary by message ID inside PostgreSQL rather than round-tripping its timestamp through JavaScript. Cancelled output and later turns must not enter a retry or maintenance prompt. Streamed answers are capped at 8,000 characters. Suggested or dismissed memories never enter prompts, and automatic summaries never cross sessions.
- Conversation maintenance advances `summary_through_at` and inserts memory suggestions in one monotonic SQL statement. An older or duplicate maintenance request must become a no-op and must not write stale memory suggestions.
...
- All AI requests must use Vercel AI Gateway credentials and endpoints. Model IDs such as `openai/gpt-5-mini` are Gateway `creator/model` routing identifiers, not direct provider integrations; do not add provider API keys, SDK clients, or endpoints. Treat HTTP `402` as a Gateway balance/budget error, while `429` remains rate limiting and must not be inferred as insufficient balance from error text.
...
- Grammar tables and core compatibility data are created from `shared/db/sql/schema.sql`; expanded learning content is seeded by `shared/db/sql/grammar-content.sql`. Together they provide 7 knowledge dimensions, 46 taxonomy nodes, 351 active learning units, 27 structured comparison sets, 10 stable error types, 5 configurable learning stages, and 19 learning modules. Eleven legacy comparison/error `grammar_points` remain as migrated compatibility records, preserving their IDs and user references. Do not reintroduce destructive grammar-point cleanup.
