...
- Normalized feedback issues store root/secondary role, confidence, evidence span, and affected dimensions. `/api/review/today` adds objective-level recommendations, while `/api/practice/metrics` and `/grammar/quality` expose aggregate generation quality without reference answers or user text.
