...
- `shared/db/sql/grammar-content.sql` expands the curriculum to 351 active learning units, 27 structured comparison sets, and 19 non-empty learning modules. `shared/db/sql/seed.sql` applies both SQL files for local setup.
...
- Adaptive grammar practice stores sessions, exercises, attempts, normalized attempt issues, mastery evidence, per-skill compatibility state, and per-sense objective state. The planner uses objective weakness, recent errors, hint dependence, answer exposure, elapsed time, and prerequisite evidence; legacy per-skill prerequisite evidence remains readable. The old generate/submit endpoints remain compatibility APIs, while `/api/practice/sessions` is the primary product flow.
- New session items use only two learner-facing forms: deterministic/validated choice and complete-sentence Chinese-to-Japanese translation. Old repair, register rewrite, and contextual response rows remain readable for history compatibility, while pending old intents are converted before generation. The compatibility generate endpoint emits translation tasks rather than the retired open text forms.
- Normalized feedback issues store root/secondary role, confidence, evidence span, and affected dimensions. `/api/review/today` exposes at most one user-facing progress item per grammar point; objective-level mastery remains available as a nested breakdown for adaptive planning, and a review record takes precedence over a separate recommendation. Started grammar that is not `mastered` contributes to `pendingCompletionCount`. A mastered record contributes to `dueReviewCount` only after its `next_review_at` has arrived; later mastered records remain available as scheduled review items. `reviewCount` remains the compatibility total. `/api/practice/metrics` and `/grammar/quality` expose aggregate generation quality without reference answers or user text.
