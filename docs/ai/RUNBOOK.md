...
- Normalized feedback issues store root/secondary role, confidence, evidence span, and affected dimensions. `/api/review/today` exposes at most one user-facing progress item per grammar point; objective-level mastery remains available as a nested breakdown for adaptive planning, and a due review record takes precedence over a separate recommendation. Pending work and completed review are separate: incomplete/weak grammar contributes to `pendingCompletionCount`, while only due `mastered` records contribute to `dueReviewCount`; `reviewCount` remains the backward-compatible total. `/api/practice/metrics` and `/grammar/quality` expose aggregate generation quality without reference answers or user text.
