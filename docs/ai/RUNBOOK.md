...
- Normalized feedback issues store root/secondary role, confidence, evidence span, and affected dimensions. `/api/review/today` exposes at most one user-facing progress item per grammar point; objective-level mastery remains available as a nested breakdown for adaptive planning, and a review record takes precedence over a separate recommendation. The user-facing buckets use only completion status: started grammar that is not `mastered` contributes to `pendingCompletionCount`, while every `mastered` record contributes to the backward-compatible `dueReviewCount` bucket shown as `待复习`. Dates, mistake counts, mastery estimates, and answer exposure do not classify these buckets; `reviewCount` remains their total. `/api/practice/metrics` and `/grammar/quality` expose aggregate generation quality without reference answers or user text.
