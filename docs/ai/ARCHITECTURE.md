...
- Never return hidden `expected_features`, the full hint ladder, reference answers, or grammar examples for an active unanswered exercise. Generated translation and contextual prompts must contain no Japanese candidate sentence beyond the target grammar label. After submission, return specific correction feedback, but keep the predefined reference-answer set behind success or explicit reveal.
